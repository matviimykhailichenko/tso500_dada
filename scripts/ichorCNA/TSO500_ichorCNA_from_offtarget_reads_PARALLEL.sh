...
#: Date        : 07-07-2025
#: Author      : Benjamin Spiegl based on work of Raúl Mejía and Isaac Lazzeri
#: Version     : 1.1
#: Description :
...
script_dir="$( cd "$( dirname "${BASH_SOURCE[0]}" )" &> /dev/null && pwd )"  # TODO: script dir was  '/var/lib/slurm/slurmd/job1760090/'
if [ "$DEBUGGING" = "true" ]; then
  echo "this is the value of the variable script_dir in 'TSO500_ichorCNA_from_offtarget_reads_PARALLEL.sh': ${script_dir}"  # for debugging
fi
...
# --- replace the whole apply_to_each_bam() with this serial version ---
function apply_to_each_bam(){
  # Run samples sequentially on the current machine (no sbatch).
  # arg1 = single-sample pipeline bash script path
  # arg2 = input directory path (where BAM files are)
  # arg3 = /PATH/TO/ichorCNAdir
  # arg4 = number of CPU cores to use (passed to the single-sample script)

  single_sample_ichor_script="${1}"
  if [ ! -f "${single_sample_ichor_script}" ]; then
  ...
  if [ ! -d "${input_directory_path}" ]; then
    echo "ERROR - path to alleged input directory '${input_directory_path}' does not exist! Exiting.."
    exit 2
  fi

  ichor_installation_path="${3}"
  if [ ! -d "${ichor_installation_path}" ]; then
    echo "ERROR - provided path to ichorCNA directory '${ichor_installation_path}' either is inaccessible or does not exist! Exiting.."
    exit 2
  fi

  threads="${4}"

  echo "INFO - Running serially on this host: $(hostname)"
  echo "INFO - Using ${threads} CPU(s) per sample (local)."

  for sample_name in "${NAMES[@]}"; do
    timestamp=$(date +"%Y-%m-%d_%H-%M-%S")
    local_log="${input_directory_path}/${sample_name}_${timestamp}.log"

    echo "--------------------------------------------------------------------------------"
    echo "INFO - Starting ichorCNA for sample '${sample_name}' (serial, no sbatch)..."
    echo "      Log: ${local_log}"
    echo "--------------------------------------------------------------------------------"

    # Run locally, capture stdout+stderr and also see it live.
    # PIPESTATUS preserves the exit code of the script itself when using tee.
    set +e
    "${single_sample_ichor_script}" \
      "${sample_name}" \
      "${input_directory_path}" \
      "${ichor_installation_path}" \
      "${threads}" \
      "${MARK_DUPLICATES_ABSOLUTE_PATH}" \
      "${CUSTOM_TSO500_PON_PATH}" \
      2>&1 | tee -a "${local_log}"
    rc=${PIPESTATUS[0]}
    set -e

    if [ $rc -ne 0 ]; then
      echo "ERROR - Sample '${sample_name}' failed with exit code ${rc}. See log: ${local_log}"
      exit $rc
    else
      echo "INFO - Sample '${sample_name}' finished successfully."
    fi
  done
}
...
case $key in
    ...
    echo "Usage:"
    echo "------"
    echo "	bash TSO500_ichorCNA_from_offtarget_reads.sh -idir path_to_input_dir -phmm path_to_hmm"
...
echo "INFO - BAM files of these samples will be processed now SERIALLY by the TSO500 off-target reads ichorCNA pipeline:"
printf "  + %s\n" "${NAMES[@]}"

# execute the pipeline for each sample (serial, local)
...
echo "All samples processed serially (no sbatch) at: ${timestamp_now}"
