...
def transfer_results_cbmed(paths: dict, input_type: str, logger: Logger):
    ...
    checksums_humgen = dragen_cbmed_dir / f'{flowcell}_Results_HumGenNAS.sha256'
    checksums_call = (
    ...
    checksums_for_cbmed = dragen_cbmed_dir / f'{flowcell}_Results_for_CBmed.sha256'
    cmd = (
        f'cd /mnt && '
    ...
    checksums_cbmed = dragen_cbmed_dir / f'{flowcell}_Results.sha256'
...
def setup_paths(repo_root: str, input_path: Path, input_type: str, tag: str, flowcell: str, config: dict,
    ...
    paths['error_msgs'] = config.get('error_msgs', {})
    paths['tag'] = tag
...
def process_object(input_type:str, paths:dict, last_sample_queue:bool, logger:Logger):
    ...
    if input_type == 'run':
        ...
        try:
            ...
            err_msg = paths['error_msgs'].get(e.returncode, 'Unknown error')
