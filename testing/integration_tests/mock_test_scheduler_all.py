...
def setup_environment():
    with open('/mnt/NovaseqXplus/TSO_pipeline/01_Staging/pure-python-refactor/config.yaml', 'r') as file:
        ...
        mixed_seq_dir = Path(config['mixed_runs_dir'] +'_TEST')
        test_run = Path('/mnt/Novaseq/TSO_pipeline/01_Staging/pure-python-refactor/testing/integration_tests/mock/test_run_oncoservice_1')
        test_samples = Path('/mnt/NovaseqXplus/TSO_pipeline/01_Staging/pure-python-refactor/testing/integration_tests/mock/test_run_onco_nsqx')
    ...
    pending_blank = Path('/mnt/Novaseq/TSO_pipeline/01_Staging/pure-python-refactor/testing/functional_tests/scheduler/PENDING_blank.txt')
    pending_file = Path('/mnt/NovaseqXplus/TSO_pipeline/10.200.214.104_PENDING.txt')
...
def test_scheduler(setup_environment):
    ...
    for i in range(6):
        run(scheduler_call, check=True, shell=True)
